...
Deno.serve(async (req) => {
  ...
  if (tokens.size === 0) {
    await admin.from("call_push_dispatches").update({ status: "failed", last_error: "recipient_has_no_push_token", updated_at: new Date().toISOString() })
  ...
  if (!firebase.token || !firebase.projectId) {
    await admin.from("call_push_dispatches").update({ status: "failed", last_error: String(firebase.error ?? "firebase_auth_failed"), updated_at: new Date().toISOString() })
      .eq("call_id", callId).eq("event", event).eq("target_user_id", targetUserId);
  ...
  const sendResults = await Promise.all([...tokens].map(async (token) => {
    try {
      ...
      return { tokenSuffix: token.slice(-8), ok: response.ok, status: response.status, detail: response.ok ? "" : text.slice(0, 300) };
    } catch (error) {
      return { tokenSuffix: token.slice(-8), ok: false, status: 0, detail: error instanceof Error ? error.message : "FCM request failed" };
    }
  }));

  const delivered = sendResults.filter((r) => r.ok).length;
  const failureText = sendResults.filter((r) => !r.ok).map((r) => `${r.status}:${r.detail}`).join(" | ").slice(0, 1000);
  await admin.from("call_push_dispatches").update({
