...
|---|---|---|---|---|
| 2026-09-22 | Premium feed navigation hierarchy | Home feed header now keeps live World Rank and Blink Coin balance visible, opens Leaderboard/BLINK Store directly, exposes Game in the header, limits feed tabs to For You/Following, keeps Create primary, and uses Home/Reels/Market/Connect/Message as the persistent bottom navigation; post headers/metrics are simplified without changing ranking or view logic | Windows top bar exposes clickable live rank/coin shortcuts plus Game, and the desktop navigation priority mirrors Home/Reels/Aluta Market/Connect/Messages with Leaderboard/Store still directly visible in the sidebar | Presentation/navigation only. Existing Supabase contracts, feed/reel ranking, qualified/repeated views, caching, ads, coins, marketplace data, messaging and game semantics are unchanged |\n| 2026-09-20 | Startup logo PNG integrity repair | Re-encoded the corrupted Compose brand mark and xxxhdpi launcher copies from the exact supplied BLINK master so startup can decode the logo without terminating | Re-encoded the matching desktop logo from the same supplied master | Shared Gradle validation now checks PNG signatures, structure and every chunk CRC across Android, Windows and web assets before Android or desktop resource processing; no backend or product behavior changed |
