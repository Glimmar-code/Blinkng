...
|---|---|---|---|---|
| 2026-09-19 | Paystack checkout readiness | Android economy service can initialize hosted Paystack checkout for server-priced coin packs or BLINK Verified, re-verify an order after browser return, and read its server status without receiving a provider secret | Windows uses the same Supabase Edge Functions and order/status contract; checkout opens the provider URL in the system browser when wired to the Store UI | Paystack secret stays only in Supabase Edge Function Secrets. Order amount/currency come from BLINK server config, webhook signatures are verified, Paystack is re-queried before fulfillment, fulfillment is idempotent, and cash checkout remains disabled until Testlab/staging validation passes |
