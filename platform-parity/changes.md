...
|---|---|---|---|---|
| 2026-09-18 | Profile data integrity, presence privacy and real growth history | Android profile counts/posts now hydrate from direct Supabase profile data; own Liked/Saved tabs use complete account relations; other users no longer expose private Liked/Growth tabs; follower Growth uses real daily snapshots only; active-status privacy suppresses presence across profile/feed/search/messages | Desktop presence indicators now render only for truly-online users; shared backend `set_my_presence` and profile privacy state protect desktop/web consumers too; Android-only Growth/profile tabs remain native surfaces | Adds truthful daily follower snapshots and privacy-aware presence backend; no ranking, messaging content, coins, verification, moderation, or notification-delivery algorithms changed |
