...
| 2026-09-19 | XP-backed Daily Missions + progression integrity | Android shows server-owned daily missions inside the BLINK Verified/earn flow, claims mission coin+XP rewards atomically, refreshes the existing Level 1–100 profile card, and blocks repeat-action XP farming | Windows shows the same XP level/progress and daily missions, claims the same server rewards, and refreshes the canonical profile/wallet after claims | Supabase is authoritative for mission definitions, progress, claim idempotency, wallet credit and XP credit. Comments use `author_id`; reversible actions key XP to their target so unlike/re-like and unfollow/refollow cannot farm XP. Web settings also hydrate XP and expose the same mission claims |
...
Date: 2026-09-18
Feature: Opt-in Google AdMob rewarded video for earning Blink Coins
Android behavior: The existing private "Earn Blink Coin" action can explicitly open a Google Mobile Ads rewarded ad. Debug builds use Google's rewarded test unit; release builds use Blink's production rewarded unit. A completed SDK reward callback starts an authenticated one-time server claim. The server applies the current economy policy: 10 base coins per completed ad plus the configured 5/10/15-ad milestone bonus, with the 15-ad daily cap.
...
Backend/shared behavior preserved: Blink Coin balance, transaction ledger, store/VIP spending, ownership and permissions remain server-authoritative in Supabase. Reward values and milestone totals are server-controlled, claims are authenticated/idempotent, and the RPC contract is shared backend infrastructure rather than Android-only coin logic.
...
Backend/shared behavior preserved: Blink account, feed/reel ranking, messaging and moderation remain unchanged. Rewarded-ad wallet credit follows the current server-controlled economy policy; the SSV database fields remain additive audit metadata and do not change Windows coin spending or shared wallet semantics.
