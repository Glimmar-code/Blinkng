...
# FeedPost model: canonical repost state + distribution attribution.
...
# Supabase transport: parse repost metadata, toggle repost, fetch game-only ranking.
...
# PostCard: attribution + dedicated Repeat action, separate from external share.
...
# FeedScreen: route repost action to each post card.
...
# ViewModel: separate game ranking and persist repost UI state.
...
# MainActivity: only the Home/Feed Game tab receives the separate game ranking.
...
replace_once(
    ...
    "MainActivity FeedScreen repost wiring",
)
